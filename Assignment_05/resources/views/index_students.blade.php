...
@section('content')
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal fade" id="create_students" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            {{--Mail Send --}}
            <div class="modal fade" id="mail_students" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" style="margin-top: 15vh;">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Mail Student</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body" style="padding:0px 0 40px !important;">
                    
                                <!-- Display Validation Errors -->
                                @include('common.errors')
                            
                                <!-- New Student Form -->
                                <form name="mail_form" action="{{ url('send-email/' ) }}" method="GET"  class="form-horizontal" style="width:400px; margin:0 auto;">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-12">
                                            <input hidden type="number" name="phone" placeholder="Phone number"  class="form-control @error('phone') is-invalid @enderror">
                                            <span id="phoneError"></span>
                                        </div>
                                        <div class="col-sm-12">
                                            <input hidden type="text" name="address" placeholder="Address"  class="form-control @error('address') is-invalid @enderror">
                                            <span id="addressError"></span>
                                        </div>
                                        <div class="col-sm-12">
                                            {{--<label for="majorlist">Choose a major</label>--}}
                                            <select hidden name="major" style="width:209px; margin:20px 0 10px; height:30px; outline:none;">
                                                @foreach ($majors as $major)
                                                <option value="{{ $major->id }}">{{$major->name}}</option>   
                                                @endforeach
                                            </select>
...
<script>
    ...
    //DELETE 
        ...
        //Mail alert
            var mailForm = document.forms['mail_form']
            var mailName = mailForm['studentName'];
            var mailPhone =  mailForm['phone'];
            var mailEmail = mailForm['email'];
            var mailAddress = mailForm['address'];
            var mailMajor = mailForm['major'];
            function mailBtn(getid)
            {
                document.cookie = 'search=0';
                console.log("update");
                console.log(getid);
                ...
                axios.get('api/index_students/'+getid)
                    .then(response => {
                        ...
                        mailName.value = response.data.name;
                        mailPhone.value = response.data.phone;
                        mailEmail.value = response.data.email;
                        mailAddress.value = response.data.address;
                        mailMajor.value = response.data.major;
            ...
            function fixData(data, majorText)
            {
                ...
                tableBody.innerHTML +=
                    '<tr>'+
                        ...
                        '<button class="btn btn-sm btn-danger" style="margin:0px 5px;" onclick="deleteBtn('+data.id+')">Delete</button>'+
                        '<button class="btn btn-sm btn-primary" style="margin-left:5px;" onclick="mailBtn('+data.id+')" data-bs-toggle="modal" data-bs-target="#mail_students">Mail</button></td>'+
                    '</tr>';
            }
